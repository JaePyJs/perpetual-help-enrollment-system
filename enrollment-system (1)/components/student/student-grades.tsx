...
// Import API functions for backend integration
import {
  fetchStudentGrades,
  fetchAcademicTerms,
  fetchGpaData,
      ...
      try {
        ...
        try {
          ...
          if (
            !termsResponse.error &&
            termsResponse.data &&
            termsResponse.data.length > 0
          ) {
            setTerms(termsResponse.data);
          }

          // Try to fetch GPA data
          const gpaResponse = await fetchGpaData();
          if (
            !gpaResponse.error &&
            gpaResponse.data &&
            gpaResponse.data.length > 0
          ) {
            setGpaHistory(gpaResponse.data);
          }
          ...
          const gradesResponse = await fetchStudentGrades(
            academicYear,
            semester
          );
          if (
            ...
            setGrades((prevGrades) => ({
              ...prevGrades,
              [selectedTerm]: gradesResponse.data || [],
            }));
          }
        } catch (apiError) {
      ...
      <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6 text-red-800 dark:text-red-300">
        <div className="flex flex-col gap-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold">Error Loading Grades</h3>
          </div>
          <p>{error}</p>
          <button
            onClick={() => window.location.reload()}
            className="self-start px-4 py-2 bg-red-100 dark:bg-red-800 text-red-800 dark:text-red-200 rounded-md hover:bg-red-200 dark:hover:bg-red-700 transition-colors"
          >
            Try Again
          </button>
        </div>
