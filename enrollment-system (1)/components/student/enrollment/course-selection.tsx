...
import { BookOpen, Calendar, Clock, Filter, Search, User, X, ChevronRight, Info, Plus, Trash2 } from "lucide-react"
...
export function CourseSelection() {
  ...
  const [courseDetailsOpen, setCourseDetailsOpen] = useState<string | null>(null)
  ...
  const courses: Course[] = [
    ...
    },
    {
      ...
      credits: 3,
      description:
        "Introduction to artificial intelligence concepts and techniques. Topics include search algorithms, knowledge representation, machine learning, neural networks, and natural language processing.",
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    // Days filter
    const matchesDays = selectedDays.length === 0 || selectedDays.some((day) => course.schedule.days.includes(day))
    ...
    <div className="flex flex-col">
      <div className="border-b">
        <div className="container flex items-center justify-between py-4">
          <div>
            <h1 className="text-2xl font-poppins font-bold">Course Selection</h1>
            <p className="text-sm text-muted-foreground">Select courses for Fall 2023 semester</p>
          </div>
                        ...
                        {["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"].map((day) => {
                          ...
                          return (
                            <Button
                              ...
                              size="sm"
                              className={isSelected ? "bg-primary hover:bg-primary-600" : ""}
                              onClick={() => {
                                if (isSelected) {
                ...
                <ScrollArea className="h-[500px]">
                  {filteredCourses.length === 0 ? (
                    <div className="flex h-32 flex-col items-center justify-center p-4 text-center">
                      <p className="text-sm text-muted-foreground">No courses match your search criteria</p>
                      <Button
                        variant="link"
                        onClick={() => {
                          setSearchQuery("")
                          ...
                          <div className="flex items-start justify-between">
                            <div>
                              ...
                              <div className="mt-1 text-sm text-muted-foreground">
                                {course.credits} {course.credits === 1 ? "Credit" : "Credits"} • {course.instructor}
                              </div>
                            ...
                            <div className="flex gap-2">
                              <Dialog
                                ...
                                onOpenChange={(open) => setCourseDetailsOpen(open ? course.id : null)}
                              >
              ...
              <CardContent>
                {selectedCourses.length === 0 ? (
                  <div className="flex h-32 flex-col items-center justify-center rounded-lg border border-dashed p-4 text-center">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">Your selected courses will appear here</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {selectedCourses.map((course) => (
                      <div key={course.id} className="flex items-start justify-between rounded-lg border p-3">
                        <div>
