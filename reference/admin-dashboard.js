...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('new-password')?.addEventListener('input', function() {
    checkPasswordStrength(this.value, 'edit-strength-indicator', 'edit-strength-text');
  });
});
...
function searchUsers() {
  ...
  setTimeout(() => {
    const users = [
      ...
      { id: '2', name: 'Jane Smith', email: 'jane@school.edu', type: 'student' },
  ...
  users.forEach(user => {
    ...
    userElement.innerHTML = `
      ...
      <button class="${action === 'remove' ? 'remove-btn' : 'modify-btn'}" 
              onclick="${action === 'remove' ? `removeUser('${user.id}')` : `loadUserForEdit('${user.id}')`}">
