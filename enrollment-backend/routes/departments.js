...
router.get("/", async (req, res) => {
  try {
    const departments = await Department.find()
      ...
      .populate("programs.coordinator", "name email");
...
router.get("/:code", async (req, res) => {
  try {
    ...
    const subjects = await Subject.find({ department: code })
      .sort({ yearLevel: 1, semester: 1 });
...
router.post("/", auth, checkRole(["admin"]), async (req, res) => {
  try {
    const { code, name, description, dean, chairperson, contactInfo } = req.body;
...
router.put("/:code", auth, checkRole(["admin"]), async (req, res) => {
  try {
    const { code } = req.params;
    const { name, description, dean, chairperson, contactInfo, status } = req.body;
...
router.post("/:code/programs", auth, checkRole(["admin"]), async (req, res) => {
  try {
    const { code } = req.params;
    const { programCode, programName, description, years, coordinator } = req.body;
    ...
    const programExists = department.programs.some(p => p.code === programCode);
    if (programExists) {
...
router.put("/:code/programs/:programCode", auth, checkRole(["admin"]), async (req, res) => {
  try {
    ...
    // Find program index
    const programIndex = department.programs.findIndex(p => p.code === programCode);
    if (programIndex === -1) {
    ...
    if (description) department.programs[programIndex].description = description;
    if (years) department.programs[programIndex].years = years;
    if (coordinator) department.programs[programIndex].coordinator = coordinator;
