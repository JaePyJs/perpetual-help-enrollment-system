...
// Get all academic years (public, only non-sensitive fields)
router.get("/years", async (req, res) => {
  try {
    const academicYears = await AcademicYear.find()
      .select("name startDate endDate semesters isCurrentYear")
      .sort({ startDate: -1 });
...
// Get current academic year and semester (public, only non-sensitive fields)
...
router.post("/years", auth, checkRole(["admin"]), async (req, res) => {
  try {
    const { name, startDate, endDate, semesters, isCurrentYear } = req.body;
    ...
    if (existingYear) {
      return res
        .status(400)
        .json({ message: "Academic year with this name already exists" });
    }
...
router.put("/years/:id", auth, checkRole(["admin"]), async (req, res) => {
  try {
    const { id } = req.params;
    const { name, startDate, endDate, semesters, isCurrentYear, status } =
      req.body;
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const semesterExists = academicYear.semesters.some(
        (s) => s.name === name
      );
      if (semesterExists) {
        return res
          .status(400)
          .json({ message: "Semester already exists for this academic year" });
      }
...
router.put(
  "/years/:id/semesters/:semesterName",
  auth,
  checkRole(["admin"]),
  async (req, res) => {
    try {
      ...
      // Find semester index
      const semesterIndex = academicYear.semesters.findIndex(
      ...
      if (lateEnrollmentPeriod)
        semester.lateEnrollmentPeriod = lateEnrollmentPeriod;
      ...
      if (gradeSubmissionDeadline)
        semester.gradeSubmissionDeadline = gradeSubmissionDeadline;
