...
const csrfConfig = {
  ...
  ignorePaths: ['/api/auth/login', '/api/auth/reset-password']
};
...
const verifyCsrfToken = (req, res, next) => {
  ...
  const token = req.headers[csrfConfig.tokenName.toLowerCase()] || 
  ...
  if (!token || !expectedToken || token !== expectedToken) {
    console.warn(`CSRF token validation failed: ${req.method} ${req.originalUrl}`);
